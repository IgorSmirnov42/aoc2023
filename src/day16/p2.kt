...
private fun solve(grid: List<String>) {
    ...
    for (col in grid[0].indices) {
        ...
        dfs(0, col, 1, grid, used)
        maxAns = maxOf(maxAns, used.sumOf { it.count { it.any { it } } })
    }
    for (col in grid[0].indices) {
        val used = Array(grid.size) { Array(grid[0].length) { BooleanArray(4) { false } } }
        dfs(grid.lastIndex, col, 3, grid, used)
        maxAns = maxOf(maxAns, used.sumOf { it.count { it.any { it } } })
    }
    for (row in grid.indices) {
        val used = Array(grid.size) { Array(grid[0].length) { BooleanArray(4) { false } } }
        dfs(row, 0, 0, grid, used)
        maxAns = maxOf(maxAns, used.sumOf { it.count { it.any { it } } })
    }
    for (row in grid.indices) {
        val used = Array(grid.size) { Array(grid[0].length) { BooleanArray(4) { false } } }
        dfs(row, grid.lastIndex, 2, grid, used)
        maxAns = maxOf(maxAns, used.sumOf { it.count { it.any { it } } })
    }
    println(maxAns)
}
